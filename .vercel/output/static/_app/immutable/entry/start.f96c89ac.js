import { o as me, t as we } from '../chunks/index.113bb01f.js';
import {
	S as Ge,
	a as Je,
	I as M,
	...
	P as Me,
	h as Ze
} from '../chunks/singletons.12c81194.js';
function Qe(t, r) {
	return t === '/' || r === 'ignore'
		? t
		: r === 'never'
...
function Ot(t, r) {
	...
	async function X({ url: e, params: s, branch: n, status: o, error: a, route: c, form: p }) {
		...
		return (
			(!d.url || e.href !== d.url.href || d.error !== a || (p !== void 0 && p !== H.form) || L) &&
				(b.props.page = {
					error: a,
					params: s,
	...
	async function ue({ loader: e, parent: s, url: n, params: o, route: a, server_data_node: c }) {
		...
		return {
			...
			loader: e,
			server: c,
			universal: (L = b.universal) != null && L.load ? { type: 'data', data: p, uses: _ } : null,
	...
	async function ie({ status: e, error: s, url: n, route: o }) {
		...
		const _ = await ue({
			...
			b = { node: await h(), loader: h, universal: null, server: null, data: null };
	...
	function Xe() {
		...
		const n = new IntersectionObserver(
			(c) => {
				for (const p of c)
					p.isIntersecting && (oe(se(new URL(p.target.href))), n.unobserve(p.target));
			},
			{ threshold: 0 }
		);
		function o(c, p) {
...
async function He(t, r) {
	...
	if (
		((h = i.headers.get('content-type')) != null && h.includes('text/html') && (await K(t)), !i.ok)
	)
